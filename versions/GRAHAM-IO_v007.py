...
def registrar_log(pasta_nome, mensagem_usuario, resposta_ai):
    """Grava a interação em um arquivo de texto dentro da pasta do usuário"""
    try:
        caminho_log = os.path.join(DATA_PATH, pasta_nome, "log_auditoria.txt")
        timestamp = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
        
        log_entry = (
            f"--- Interação em {timestamp} ---\n"
            f"USUÁRIO: {mensagem_usuario}\n"
            f"GRAHAM-IO: {resposta_ai}\n"
            f"{'-'*40}\n"
        )
        
        with open(caminho_log, "a", encoding="utf-8") as f:
            f.write(log_entry)
        print(f"DEBUG: Log de auditoria atualizado em {pasta_nome}.")
...
def carregar_dados_usuario(nome_dito):
    try:
        ...
        # Retornamos também o nome da pasta para facilitar o log
        return perfil, produtos, saldo_real, pasta_encontrada
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if nome_dito:
        perfil, produtos, saldo, pasta_id = carregar_dados_usuario(nome_dito)
        ...
        if perfil:
            ...
            REGRAS: Máximo 4 frases. Segue Benjamin Graham. Mencione o saldo real no início.
            """
            historico_conversa = [{"role": "system", "content": SYSTEM_PROMPT}]
            
            falar(f"Acesso concedido. {perfil['nome']}, o teu saldo consolidado é de {saldo:.2f} reais. Como posso ajudar?")
            ...
            while True:
                ...
                if pergunta:
                    ...
                    resposta = pensar(pergunta, historico_conversa)
                    
                    # REGISTRO DE AUDITORIA
                    registrar_log(pasta_id, pergunta, resposta)
